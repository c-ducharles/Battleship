...
def blockaround():
    ...
    for l in range (10) :
        for c in range (10) :
            if (cases1[l][c] < -9) and n < 7 :
                plateau1.create_image(50*(c)+25, 50*(l)+25,image=croix)
                plateau1.pack()
            
            if (cases2[l][c] < -9) and n > 5:
                plateau2.create_image(50*(c)+25, 50*(l)+25,image=croix)
                plateau2.pack()
...
def core(event) : #programme principal qui réagit à chaque clic
    ...
    if mode : #si mode est vrai alors le programme place les batteaux 
        ...
        if selection : #permet d'inerverser les joueurs 
            ...
            elif (cases1[l][c] == -10) or (cases1[l][c] == 0) :
            ...
            elif (cases2[l][c] == -10) or (cases2[l][c] == 0) :
...
def construction3(): #sert à relancer le jeu, rappel le jeu et detruit l'ancien
    global cases1, cases2,n, z, h, q, t0, t1, t2, t3,t4, t5, t6, t7, t8, t9, bouton_blanchir, selection, mode, clic
    for i in range(10): 
        for j in range(10):
            cases1[i][j]=0
